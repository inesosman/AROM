...
public class ClassHierarchy2 {
	...
	private static void print(Set<String> liste, Node<OWLClass> parent, OWLReasoner reasoner, int depth, HashSet<Node<OWLClass>> visited) {
		...
		if (!visited.contains(parent) ) {
			...
			for (Iterator<OWLClass> iterator = parent.getEntities().iterator(); iterator.hasNext();j++) {
				...
				System.out.println("N° " + depth + "  :  " + cls.getIRI().getFragment() + " ("+j+")");

				liste.add(String.valueOf(depth));
			}

			for (Node<OWLClass> child : reasoner.getSubClasses(parent.getRepresentativeElement(), true)) {
				assert child != null;

				print(liste, child, reasoner, depth + 1, visited);
			}
		} 
	}
	...
	{   
		...
		print(levelsList, topClassNode, reasoner, 0, new HashSet<Node<OWLClass>>());
