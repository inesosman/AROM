...
The following figures show the merged class in our output ontology that resulted from the merging of the three _LargeBio_ ontologies. The merged class captures all knowledge (axioms and expressions) defining the three equivalent classes. It has a unique code (__Code\_19351__) as a (local) name, and all entities (existing in its description), that have been merged, also have their corresponding unique code as a (local) name. That is, the class __Segment\_of\_skin__ was merged with its equivalent classes to form the class __Code\_3840__; The class __Skin__ was merged with its equivalent classes to form the class __Code\_24805__; And the class __Head__ was merged with its equivalent classes to form the class __Code\_17698__. Besides, the merged class have three added labels (framed in red), which mention the original (local) names of the classes that have been merged. We attached each name to its ontology number (ID) to directly see from which ontology it originates.
